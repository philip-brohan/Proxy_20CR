...
# Convolutional Variational Autoencoder for ERA5 Tm2m
...
nTrainingImages = 14026  # Max is 14026
nTestImages = 1558  # Max is 1558
...
with strategy.scope():
    ...
    if args.epoch > 0:
        weights_dir = (
            "%s/Proxy_20CR/models/DCVAE_single_ERA5_T2m/" + "Epoch_%04d"
        ) % (
            os.getenv("SCRATCH"),
            args.epoch,
        )
...
def save_state(model, epoch, loss):
    save_dir = ("%s/Proxy_20CR/models/DCVAE_single_ERA5_T2m/" + "Epoch_%04d") % (
        os.getenv("SCRATCH"),
        epoch,
    )
