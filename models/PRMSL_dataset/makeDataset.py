...
def getDataset(purpose, nImages=None):
    ...
    inFiles = os.listdir(
        "%s/Proxy_20CR/datasets/20CR2c/prmsl/%s" % (os.getenv("SCRATCH"), purpose)
    )
    ...
    inFiles = [
        "%s/Proxy_20CR/datasets/20CR2c/prmsl/%s/%s" % (os.getenv("SCRATCH"), purpose, x)
        for x in inFiles
    ]
