...
Suppose we have a representation of the atmospheric state :math:`w`, and some observations :math:`y^o`. We'd like to know what values of :math:`w` are consistent with :math:`y^o` - we want the probability distribution function :math:`p(w|y^o)` (or at least the value of :math:`w` that maximises this probability - the best-estimate atmospheric state given the observations).

From Bayes' theorem:

.. math::

   p(w|y^o) = p(y^o|w)p(w)/p(y^o)

where :math:`p(w)` is our prior estimate of :math:`w` (before we got the observations) - sometimes called a "background" estimate. And :math:`p(y^o)` is our prior probability of observations :math:`y^o`.

Start by assuming :math:`p(y^o)` is constant - all values of observations are equally likely. 

To calculate :math:`p(y|w)` we need an observations operator :math:`H`: :math:`y=H(w)`. Suppose :math:`y^o` and the corresponding :math:`H(w)` contain Gaussian errors with covariance :math:`R`. Then

.. math::

   p(y^o|w) \propto \exp(-1/2(y^o-H(w))^TR^{-1}(y^o-H(w))


We also need a prior estimate :math:`p(w)` for the atmospheric state. This is a property of the model we are using to represent the atmosphere. Our atmospheric state :math:`w` will be the output of a model (call this :math:`g`):
...
* It will produce precise and accurate estimates of the atmospheric state :math:`w`. In practice this means :math:`p(y^o|w)` is small compared to alternative models.
* It will be fast, so it's cheap to generate samples of :math:`w`.
* It will be cheap and easy to estimate :math:`p(w)` - the prior estimate of the atmospheric state.

Modern `GCMs <https://en.wikipedia.org/wiki/General_circulation_model>`_ score very highly on the first of these points - they make very precise and accurate estimates of the atmospheric state vector - but nothing about them is cheap, or easy. On the other hand, simple statistical models can make fast samples of :math:`w` and :math:`p(w)` (and so allow fast estimates of the :math:`w` that maximises :math:`p(w|y^o)`) but the quality of the atmospheric states they produce are unsatisfactory.

Modern `Machine Learning (ML) <https://en.wikipedia.org/wiki/Machine_learning>`_ offers us the chance to square this circle: simple :math:`p(w)`, fast estimates of :math:`w` (and so :math:`p(w|y^o)`), and good quality atmospheric state estimates. Speed, accuracy, and precision.

A concrete example: 2-metre air temperature
-------------------------------------------

As a concrete example, we will use not the whole weather state vector, but just the 2-metre air temperature anomaly (as our :math:`w`). And we will use station observations of 2-metre temperature to constrain it.  So there is some function :math:`f()` where:
...
Where each :math:`o` is a triplet of (lat, lon, T2m anomaly) and :math:`n` is a variable - we need to be able to use any number of observations. The challenge is to use ML to make a suitable :math:`f()`, that is, one where :math:`f(y^o) = w^o`, where :math:`w^o` is the value of :math:`w` that maximises :math:`p(w|y^o)`

This example has the virtue that the observations operator :math:`H(w)` is very simple - we just interpolate :math:`w` to the locations of the observations. Also we can reasonably assume the observations errors are constant and uncorrelated, so maximising :math:`p(y^o|w)` is the same as minimising the RMS difference between :math:`H(w)` and :math:`y^o`. That is, between the observations and the field :math:`w` at the locations of the observations.

Generative model, and latent space
----------------------------------

Traditionally, we would use a GCM to generate plausible temperature fields (:math:`w`). Here, we are going to replace the GCM with a generative model: We need a model :math:`w=g(x)`, where a cheap and simple prior input distribution :math:`p(x)` produces a wide range of good quality output temperature fields (the same sort of output a good GCM might produce, or that we see in observations).
...
This function is called a generator, and the  inputs :math:`x` form a vector in 100-dimensional space (the *latent space*). There are three additional requirements on :math:`g()`: We want to be able to perturb the state, so it must be continuous - a small change in the latent space (:math:`x`) must result in a small change in the output (:math:`w`). We want to sample freely from the prior, so it must be complete - any point in the latent space (sampled from the prior) must map into a plausible temperature anomaly field. And it must be *fast* (otherwise, we could just use a GCM).

This function is specified to meet the three requirements above (good quality :math:`w`, cheap and easy :math:`p(w)`, and cheap and easy :math:`p(y^o|w)`). It would be almost impossible to code a function meeting this specification, but we don't have to write the function, we can *learn* it.
...
* It makes good quality :math:`w` as output (:math:`p(w=g(x)|x)\approx 1`), because we train it on examples of good quality :math:`w` - here temperature anomaly fields from `ERA5 <https://www.ecmwf.int/en/forecasts/datasets/reanalysis-datasets/era5>`_.
* It has a cheap and easy :math:`p(w)`, because :math:`p(w)=p(w|x)p(x)`, :math:`p(w|x)\approx 1`, and we train :math:`p(x)` (and so :math:`p(w)`) to be a multi-variate unit normal distribution.
...
So as long as as the VAE can be trained successfully on good quality data (examples of the sort of :math:`w` that we want it to produce), the generator learned by the VAE will be exactly the function :math:`g()` that we need. And it does work: training a simple VAE on 40 years of daily T2m anomalies (from ERA5) gives decent results after only about 10 minutes (on a single v100 GPU).
...
We want to find the value of :math:`w` which maximises:

.. math::

   p(w|y^o) & = p(y^o|w)p(w)/p(y^o) \\
            & \propto \exp(-{1\over2}(y^o-H(w))^TR^{-1}(y^o-H(w))p(w) \\
            & \approx \exp(-{1\over2}(y^o-H(w))^2)p(w)

and :math:`w=g(x)` so we need the value of x which maximises:

.. math::

             \exp(-{1\over2}(y^o-H(g(x)))^2)p(g(x)|x)p(x)

We have :math:`g(x)` - the generator we learned with the VAE; :math:`p(w=g(x)|x)\approx 1`; :math:`p(x)` is a unit normal distribution; and :math:`H(g(x))` is very simple - we just interpolate the generator output to the locations of the observations. So we can calculate this for any :math:`x` and finding the maximum is just a matter of optimisation.
...
Data assimilation means finding a weather field :math:`w` that is the best fit to a set of observations :math:`y^o`. This means maximising

.. math::

   p(y^o|w)p(w)

and this is difficult because :math:`p(w)`, the prior probability of any particular weather field, is hard to calculate. Most fields of temperature, wind etc. are physically or dynamically impossible - they don't represent weather states that could occur in reality (i.e. :math:`p(w)=0`). So we can't find a maximum of this expression by searching in w - we'd need a fast functional representation of :math:`p(w)`, and we don't have one.

Machine learning provides us with a function :math:`w=g(x)` that lets us transform the problem into the latent space :math:`x`, where :math:`p(x)` is an easy function. So maximising
