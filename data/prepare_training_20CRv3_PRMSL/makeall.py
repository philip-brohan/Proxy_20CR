...
# Get one data file every 1 days+3 hours over the selected years
...
def is_done(year, month, day, hour, group):
    op_file_name = (
        ("%s/Proxy_20CR/datasets/20CRv3/%s/" + "%s/%04d-%02d-%02d:%02d.tfd")
    ) % (
        os.getenv("SCRATCH"),
        'PRMSL',
        group,
...
while current_day <= end_day:
    if count % 10 == 0:
        ...
        if not is_done(
            ...
            f.write(cmd)
    current_day = current_day + datetime.timedelta(hours=27)
    count += 1
